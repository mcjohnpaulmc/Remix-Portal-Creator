...
router.post("/subdomains", async (req, res) => {
  ...
  if (action === "update") {
    ...
    if (!canAccessPortal(portal, adminEmail, isSuperAdmin, db.users || [])) {
    ...
    if (!canAccessPortal(portal, (req as any).adminEmail, isSuperAdmin, db.users || [])) {
      return res.status(403).json({ error: "You do not have permission to modify this portal." });
    }
    ...
    if (deletedPortal && !canAccessPortal(deletedPortal, (req as any).adminEmail, isSuperAdmin, db.users || [])) {
...
router.post("/refresh-dns", async (req, res) => {
  ...
  const pendingPortals = (db.subdomains || []).filter(
    s => !s.isDummy && s.domain && s.dnsStatus === "pending" && canAccessPortal(s, adminEmail, isSuperAdmin, db.users || [])
  );
